...
/**
 * ListGUI.java - This class is the View of the Application.  It has input
 * fields for user input, buttons to control the underlying data structure
 * (AddressList) and a TextArea that allows for application output.
 *
...
public class ListGUI extends JFrame {
    ...
    // Used to display output from the button
    ...
    private void initOutputArea() {
        ...
        pane.setBounds( OUTPUT_X_POS, OUTPUT_Y_POS,
    ...
    private void initDataPanel() {
        ...
        dataPanel.setBounds( DATA_X_POS, DATA_Y_POS,
        ...
        for ( int i = 0; i < labels.length; i++ ) {
            ...
            dataPanel.add(
                    createLabel( labels[i], JLabel.RIGHT, labelDimension ), gbc
            );
        }
    ...
    /** Initialize and add buttons that allow operations on the AddressList */
    private void initButtonPanel() {
        JPanel buttonPanel = new JPanel( new GridBagLayout() );
        buttonPanel.setBounds( BUTTON_X_POS, BUTTON_Y_POS,
        ...
        button.addActionListener(
                ( e ) -> list.addToFront(
                        nameField.getText(), telField.getText(),
                        emailField.getText(), addrField.getText(), dobField.getText()
                )
        );
        ...
        button.addActionListener(
                ( e ) -> outputArea.append( "List Size: " + list.sizeOf() + "\n" )
        );
    ...
    private void initSearchPanel() {
        ...
        searchPanel.setBounds( SEARCH_X_POS, SEARCH_Y_POS, SEARCH_WIDTH, SEARCH_HEIGHT );
        ...
        button.addActionListener( ( e ) -> outputArea.append(
                "Phone number for " + nameField.getText() + ": " +
                        list.phoneNumberByName( nameField.getText() ) + "\n"
        ) );
        ...
        button.addActionListener( ( e ) -> outputArea.append(
                "Email for " + nameField.getText() + ": " +
                        list.emailByName( nameField.getText() ) + "\n"
        ) );
        ...
        button.addActionListener( ( e ) -> outputArea.append(
                "DOB for " + nameField.getText() + ": " +
                        list.dobByName( nameField.getText() ) + "\n"
        ) );
        ...
        button.addActionListener( ( e ) -> outputArea.append(
                "Name for " + telField.getText() + ": " +
                        list.nameByPhoneNumber( telField.getText() ) + "\n"
        ) );
    ...
    /**
     * Add a component to a JPanel with the provided GridBagConstraints and x,y
     * position.  JPanel is assumed to be using a GridBagLayout LayoutManager.
     *
     * @param target JPanel to add obj to
     * @param obj    Component to add to target
     * @param gbc    GridBagConstraints to use when adding the Component
     * @param x      int x position of the Component in the GridBagLayout
     * @param y      int y position of the Component in the GridBagLayout
     */
    private void addToPanel( JPanel target, Component obj,
